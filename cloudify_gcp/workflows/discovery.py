...
GCP_TYPE = 'cloudify.nodes.gcp.Gcp'
...
def discover_and_deploy(node_id=None,
    ...
    # Refresh the GCP_TYPE nodes list..
...
def get_gcp_account_node_id(nodes):
    ...
    """
    for node in nodes:
        if GCP_TYPE in node.type_hierarchy:
            return node.id
    raise NonRecoverableError(
        'The deployment has no nodes of type {t}.'.format(t=GCP_TYPE))
