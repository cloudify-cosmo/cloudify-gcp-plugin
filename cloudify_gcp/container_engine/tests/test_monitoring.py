...
# Local imports
from cloudify_gcp.container_engine import monitoring
...
class TestGCPMonitoringService(TestGCP):
    def test_create(self, mock_build, *args):
        monitoring.set_monitoring_service('test',
                                          'valid_name',
                                          additional_settings={})

        mock_build().projects().zones().clusters(
        ).monitoring.assert_called_once_with(
            body={'monitoringService': 'test'},
